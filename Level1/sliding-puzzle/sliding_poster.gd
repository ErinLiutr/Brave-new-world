...
func _ready():
	...
	for n in range(12):
		...
		for y in range(4):
			if map[y].find(n) != -1:
				img[n].position = Vector2(map[y].find(n)*24, y*24)
...
func move(dir):
	for y in range(4):
			if map[y].find(11) != -1:
				...
				img[map[y][map[y].find(11)]].position = Vector2(pos.y*24, pos.x*24)
				if img[map[pos.x][pos.y]].position != Vector2(map[y].find(11)*24, y*24):
					move = dir
